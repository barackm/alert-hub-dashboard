...
const ResponseMessage: React.FC<ResponseMessageProps> = ({
  ...
  return (
    <div className='absolute inset-0 z-50 flex flex-col bg-gray-800'>
      {editMode ? (
        <div className='h-12 flex items-center justify-between px-6 text-white text-sm bg-gray-800 pt-[70px]'>
          <button className='text-blue-500 font-medium py-4' onClick={() => setEditMode(false)}>
            Cancel
          </button>
          <span className='font-semibold'>Reply</span>
          <button className='text-blue-500 font-medium py-4' onClick={sendUssd}>
            Reply
          </button>
        </div>
      ) : null}
